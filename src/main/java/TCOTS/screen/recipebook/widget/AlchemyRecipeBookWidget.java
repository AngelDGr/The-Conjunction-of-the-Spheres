...
//        this.cachedInvBlockChangeCount = craftingScreenHandler.getChangeCount();
    ...
    public void renderRecipeBookBackground(DrawContext context, int width, int height, int i, int j){
        if (open) {
            context.getMatrices().push();
    ...
    public void reset(){
        ...
        if(this.tabButtons.isEmpty()){
            ...
            //Bombs
            this.tabButtons.add(2,new AlchemyRecipeGroupButton(TCOTS_Items.GRAPESHOT.getDefaultStack(), AlchemyTableRecipeCategory.BOMBS_OILS));
            //Misc
