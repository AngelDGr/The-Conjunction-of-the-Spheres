...
public class DrownerModel extends GeoModel<DrownerEntity> {
    ...
    public void setCustomAnimations(DrownerEntity entity, long instanceId, AnimationState<DrownerEntity> animationState) {
        ...
        if(head!=null) {
            CoreGeoBone wholeBody = getAnimationProcessor().getBone("UpperBody_head").getParent();
            //Swimming
            if (entity.isSubmergedInWater()) {
//                wholeBody.setRotX(180);
                head.setRotX((entity.getPitch() + 70) * MathHelper.RADIANS_PER_DEGREE);
            }
            //OnLand
            else {
