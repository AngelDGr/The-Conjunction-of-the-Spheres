...
public class TrollGossipsFriendship {
    private static final Logger LOGGER = LogUtils.getLogger();
    private final Map<UUID, TrollGossipsFriendship.Reputation> entityReputation = Maps.newHashMap();
    ...
    public Map<UUID, Object2IntMap<TrollGossipType>> getEntityReputationAssociatedGossips() {
        ...
        this.entityReputation.keySet().forEach(uuid -> {
            TrollGossipsFriendship.Reputation reputation = this.entityReputation.get(uuid);
    ...
    private TrollGossipsFriendship.Reputation getReputationFor(UUID target) {
        return this.entityReputation.computeIfAbsent(target, uuid -> new TrollGossipsFriendship.Reputation());
    }

    public int getReputationFor(UUID target, Predicate<TrollGossipType> gossipTypeFilter) {
        TrollGossipsFriendship.Reputation reputation = this.entityReputation.get(target);
...
//    }
    ...
    public void startGossip(UUID target, TrollGossipType type, int value) {
        TrollGossipsFriendship.Reputation reputation = this.getReputationFor(target);
    ...
    public void remove(TrollGossipType type) {
        Iterator<TrollGossipsFriendship.Reputation> iterator = this.entityReputation.values().iterator();
        while (iterator.hasNext()) {
            TrollGossipsFriendship.Reputation reputation = iterator.next();
            reputation.remove(type);
