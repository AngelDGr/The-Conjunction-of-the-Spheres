...
public abstract class MonsterNestSpawnerLogic{
...
//    private DataPool<MobSpawnerEntry> spawnPotentials = DataPool.empty();
    protected DataPool<MobSpawnerEntry> spawnPotentials = new DataPool<MobSpawnerEntry>(List.of());
    @Nullable
    protected MobSpawnerEntry spawnEntry;
    private double rotation;
    private double lastRotation;
    protected int minSpawnDelay = 50;
    protected int maxSpawnDelay = 100;
    ...
    public void clientTick(World world, BlockPos pos) {
        if (!this.isPlayerInRange(world, pos)) {
            ...
            world.addParticle(ParticleTypes.SPLASH, d, e, f, 0.0, 0.0, 0.0);
    ...
    public void readNbt(@Nullable World world, BlockPos pos, NbtCompound nbt) {
        ...
        if (bl2 = nbt.contains("SpawnPotentials", NbtElement.LIST_TYPE)) {
            ...
            this.spawnPotentials = MobSpawnerEntry.DATA_POOL_CODEC.parse(NbtOps.INSTANCE, nbtList).resultOrPartial(error -> LOGGER.warn("Invalid SpawnPotentials list: {}", error)).orElseGet(
                    (Supplier<? extends DataPool<MobSpawnerEntry>>) new DataPool<MobSpawnerEntry>(List.of())
            );
        } else {
