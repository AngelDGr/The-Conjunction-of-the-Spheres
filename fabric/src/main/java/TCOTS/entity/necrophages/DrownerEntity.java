...
//xTODO: Add spawn
    ...
    public DrownerEntity(EntityType<? extends DrownerEntity> entityType, Level world) {
...
//        this.goalSelector.add(4, new FollowWaterHag(this, 0.75, 3,7));
    ...
    private static class WaterOrLand_MoveControl extends MoveControl {
        ...
        public void handleUnderwaterMovement(){
            ...
            double dXtoTarget = this.wantedX - this.mob.getX();
            double dYtoTarget = this.wantedY - this.mob.getY();
            double dZtoTarget = this.wantedZ - this.mob.getZ();
            double length = Math.sqrt(dXtoTarget * dXtoTarget + dYtoTarget * dYtoTarget + dZtoTarget * dZtoTarget);

            //Movement Vector
            Vec3 vec3D_movementUnderwater = new Vec3((dXtoTarget / length) * waterSpeed,
...
//                }
            ...
            if (this.operation == Operation.MOVE_TO && !this.mob.getNavigation().isDone()) {
                ...
                float hipoY = (float) (Mth.atan2(dZtoTarget, dXtoTarget) * 57.2957763671875) - 90.0F;
    ...
    private class Drowner_LandWaterAttackGoal extends MeleeAttackGoal_Animated {
        ...
        protected void attack(LivingEntity target) {
            ...
            if (drowner.getSwimmingDataTracker()) {
                ...
                if(target!=null){
                    int randomExtra = DrownerEntity.this.random.nextIntBetweenInclusive(1,51);
    ...
    private class Drowner_Attack_Lunge extends LungeAttackGoal {
        ...
        public Drowner_Attack_Lunge(PathfinderMob mob, int cooldownBetweenLungesAttacks, double lungeImpulse) {
    ...
    private class Drowner_SwimAroundGoal extends RandomSwimmingGoal{
        ...
        public Drowner_SwimAroundGoal(PathfinderMob pathAwareEntity, double d, int i) {
    ...
    public void setSpawnedPuddleDataTracker(boolean puddleSpawned) {this.entityData.set(SPAWNED_PUDDLE, puddleSpawned);}
    ...
    public void spawnGroundParticles(@NotNull LivingEntity entity) {
        ...
        if (blockState.getRenderShape() != RenderShape.INVISIBLE) {
            for(int i = 0; i < 11; ++i) {
                double d = entity.getX() + (double)Mth.randomBetween(random, -0.7F, 0.7F);
                double e = entity.getY();
                double f = entity.getZ() + (double)Mth.randomBetween(random, -0.7F, 0.7F);
    ...
    //Natural Spawn
    public static boolean canSpawnDrowner(EntityType<? extends NecrophageMonster> type, ServerLevelAccessor world, MobSpawnType spawnReason, BlockPos pos, RandomSource random) {
