...
public class WitcherPotions_Base extends PotionItem {
    ...
    public static ItemStack getStackEmptyBottle(WitcherPotions_Base item) {
        ItemStack stack_Empty=new ItemStack(TCOTS_Items_Fabric.EMPTY_WITCHER_POTION);

        if(!item.decoction){
            stack_Empty.set(DataComponents.MAX_STACK_SIZE, item.getDefaultMaxStackSize());
        }
        else {
            stack_Empty = new ItemStack(TCOTS_Items_Fabric.EMPTY_MONSTER_DECOCTION);
        }

        stack_Empty.set(TCOTS_Items_Fabric.REFILL_RECIPE, BuiltInRegistries.ITEM.getKey(item).toString());
