...
public class WitcherAlcohol_Base extends WitcherPotions_Base {
    ...
    public void appendHoverText(@NotNull final ItemStack stack, final TooltipContext context, @NotNull final List<Component> tooltip, @NotNull final TooltipFlag type) {
        tooltip.add(Component.translatable("item.tcots_witcher."+BuiltInRegistries.ITEM.getKey(this).getPath()+".tooltip").withStyle(ChatFormatting.GRAY));
        tooltip.add(CommonComponents.EMPTY);
        tooltip.add(Component.translatable("item.tcots_witcher.alcohol.tooltip.refill").withStyle(ChatFormatting.GRAY));


        tooltip.add(CommonComponents.space().append(Component.translatable(
                this.refillQuantity >1 && this.refillQuantity <6?
                "item.tcots_witcher.alcohol.tooltip.slots":
                        this.refillQuantity>1?
                                "item.tcots_witcher.alcohol.tooltip.slots2":
                                "item.tcots_witcher.alcohol.tooltip.slot",
