...
public class DragonsDreamCloud extends AreaEffectCloud {
    ...
    protected void onInsideBlock(@NotNull final BlockState state) {
        ...
        if(state.is(TCOTS_Tags.Block.IGNITING_BLOCKS) || CampfireBlock.isLitCampfire(state)){
            createExplosion();
        }
    }
    ...
    public void tick() {
        ...
        for(final Entity entity: entitiesList){
            if(entity.isOnFire() || entity.getType().is(TCOTS_Tags.Entity.IGNITING_ENTITIES)){
