...
import mors.tcots.client.geo.animation.entity.necrophage.BullvoreAnimations;
...
public class BullvoreEntity extends NecrophageMonster implements GeoEntity {
    ...
    public BullvoreEntity(final EntityType<? extends BullvoreEntity> entityType, final Level world) {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(1, new Bullvore_MeleeAttackGoal(this, 1.2D, false, 100));
    ...
    protected static class Bullvore_MeleeAttackGoal extends Goal {
        ...
        public Bullvore_MeleeAttackGoal(final BullvoreEntity mob, final double speed, final boolean pauseWhenMobIdle, final int chargeCooldown) {
            this.mob = mob;
            this.speed = speed;
        ...
        public void tick() {
            ...
            if(!mob.isCharging()) {
                ...
                if ((this.pauseWhenMobIdle || this.mob.getSensing().hasLineOfSight(target)) && this.updateCountdownTicks <= 0 && (this.targetX == 0.0 && this.targetY == 0.0 && this.targetZ == 0.0 || target.distanceToSqr(this.targetX, this.targetY, this.targetZ) >= 1.0 || this.mob.getRandom().nextFloat() < 0.05f)) {
                    ...
                    final double d = this.mob.distanceToSqr(target);
                    if (d > 1024.0) {
            ...
            final Vec3 monsterPosition = this.mob.position();
            final Vec3 monsterLookVec = this.mob.calculateViewVector(0.0f, mob.getYHeadRot()); // This gives the direction the monster is facing
            ...
            //Start Charging
            if(!this.mob.chargeCooldown && !this.mob.isCharging() && isFacingTarget){
            ...
            if(mob.isCharging()){
                ...
                this.mob.getNavigation().moveTo(this.pathCharge,this.speed);
        ...
        protected void attack(final LivingEntity target) {
            if (this.canAttack(target)) {
                ...
                if (this.mob.getRandom().nextBoolean()) {
                    mob.triggerAnim("base_controller", "attack1");
                } else {
                    mob.triggerAnim("base_controller", "attack2");
                }
    ...
    public void registerControllers(final AnimatableManager.ControllerRegistrar controllerRegistrar) {
        controllerRegistrar.add(BullvoreAnimations.mainController(this));
    }
    ...
    public float getSpeed() {
        return this.isSprinting()? super.getSpeed()*1.6f : super.getSpeed();
    }
    ...
    protected void customServerAiStep() {
        ...
        if(this.isCharging()) TCOTS_EntitiesUtil.pushAndDamageEntities(this, 14f, 1.1, 1.1, 1.5D, BullvoreEntity.class, RotfiendEntity.class);
    }
    ...
    public void setCharging(final boolean wasCharging) {
    ...
    protected void playStepSound(final @NotNull BlockPos pos, final @NotNull BlockState state) {
        if(getStepSound()!=null){
            ...
            final SoundType blockSoundGroup = state.getSoundType();
