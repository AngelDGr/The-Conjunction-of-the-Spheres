...
import mors.tcots.client.geo.animation.entity.necrophage.GraveirAnimations;
...
//    protected static final TrackedData<Boolean> PUNCHING = DataTracker.registerData(GraveirEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    ...
    public GraveirEntity(final EntityType<? extends GraveirEntity> entityType, final Level world) {
    ...
    public void registerControllers(final AnimatableManager.ControllerRegistrar controllerRegistrar) {
        controllerRegistrar.add(GraveirAnimations.mainController(this));
    ...
    public boolean doHurtTarget(final @NotNull Entity target) {
        ...
        if(target instanceof final LivingEntity living && bl && this.random.nextInt()%3==0){
...
//                player.getOffHandStack().damage(10, random, (ServerPlayerEntity) (player));
    ...
    //Spawn
    public static boolean canSpawnGraveir(final EntityType<? extends NecrophageMonster> type, final ServerLevelAccessor world, final MobSpawnType spawnReason, final BlockPos pos, final RandomSource random) {
