...
import mors.tcots.client.geo.animation.entity.ogroid.CyclopsAnimations;
...
public class CyclopsEntity extends OgroidMonster implements GeoEntity {
    ...
    public CyclopsEntity(final EntityType<? extends CyclopsEntity> entityType, final Level world) {
    ...
    private static class CyclopsMeleeAttackGoal extends MeleeAttackGoal_Animated {
        ...
        public CyclopsMeleeAttackGoal(final PathfinderMob mob, final double speed, final boolean pauseWhenMobIdle) {
    ...
    private static class CyclopsJumpAttack extends Goal {
        ...
        public CyclopsJumpAttack(final CyclopsEntity cyclops, final int minJumpCooldown, final int maxExtraRandomCooldown) {
        ...
        @Override
        public boolean canUse() {
            final LivingEntity target = this.cyclops.getTarget();
    ...
    public void registerControllers(final AnimatableManager.ControllerRegistrar controllerRegistrar) {
        controllerRegistrar.add(CyclopsAnimations.mainController(this));
    ...
    public void resetFallDistance() {
        if(isFalling()){
                    ...
                    && !this.level().getBlockState(this.blockPosition().below()).is(TCOTS_Tags.Block.NEGATES_DEVOURER_JUMP))
            {
                ...
                TCOTS_EntitiesUtil.pushAndDamageEntities(this, 6 + (fallDistance*2f), 3.0f + (fallDistance * 0.5f), 3, 3.0);
    ...
    public void handleEntityEvent(final byte status) {
        if(status==FALLING_PARTICLES){
            TCOTS_EntitiesUtil.spawnImpactParticles(this, 1.9975f + (2.8f + (getFallingDistance() * 0.5f)), this.getFallingDistance());
        } else{
    ...
    public static boolean canCyclopsSpawn(final EntityType<? extends Mob> type, final LevelAccessor world, final MobSpawnType spawnReason, final BlockPos pos, final RandomSource random) {
        final BlockPos blockPos = pos.below();
        final int radiusToSearchCyclops=80;
