...
public class GhoulEntity extends NecrophageMonster implements GeoEntity, LungeMob, TraceableEntity, GuardNestMob, ExcavatorMob {
    ...
    protected static final EntityDataAccessor<Boolean> InGROUND = SynchedEntityData.defineId(GhoulEntity.class, EntityDataSerializers.BOOLEAN);
    protected static final EntityDataAccessor<Boolean> EMERGING = SynchedEntityData.defineId(GhoulEntity.class, EntityDataSerializers.BOOLEAN);
    protected static final EntityDataAccessor<Boolean> INVISIBLE = SynchedEntityData.defineId(GhoulEntity.class, EntityDataSerializers.BOOLEAN);


    public GhoulEntity(final EntityType<? extends GhoulEntity> entityType, final Level world) {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(7, new FollowMonsterOwnerGoal(this, 0.75));

        this.goalSelector.addGoal(8, new WanderAroundGoal_Excavator(this, 0.75f,80));

        this.goalSelector.addGoal(9, new LookAroundGoal_Excavator(this));
    ...
    public boolean getExtraReasonToNotGoToNest() {
        final List<ItemEntity> listFlesh = this.level().getEntitiesOfClass(ItemEntity.class,
    ...
    public void setAnimationParticlesTicks(final int animationParticlesTicks) {
        this.animationParticlesTicks = animationParticlesTicks;
    }
    ...
    public void setReturnToGround_Ticks(final int returnToGround_Ticks) {
        this.ReturnToGround_Ticks=returnToGround_Ticks;
    }
    ...
    protected static class GhoulRegeneration extends Goal {
        ...
        public GhoulRegeneration(final GhoulEntity mob, final int ticksAttackedBeforeRegen, final int CooldownBetweenRegens, final int TimeForRegen, final float HealthPercentageToStart, final int stopTicks) {
        ...
        public void start() {
            ...
            mob.triggerAnim("base_controller", "start_regen");
        }
    ...
    protected static class Ghoul_MeleeAttackGoal extends MeleeAttackGoal_Excavator {

        public Ghoul_MeleeAttackGoal(final PathfinderMob mob, final double speed, final boolean pauseWhenMobIdle) {
            super(mob, speed, pauseWhenMobIdle, 2400, true);
        }
    ...
    protected static class GhoulGoForFlesh extends Goal {
        ...
        public GhoulGoForFlesh(final GhoulEntity ghoul, final double speed){
            this.ghoul=ghoul;
            this.speed=speed;
        }
        ...
        @Override
        public boolean canUse() {
            final List<ItemEntity> list = searchFleshList();
    ...
    private static boolean isEdibleMeat(final ItemStack stack){
    ...
    private void handleItemFromGround(final ItemEntity item){
        this.take(item, 1);
        final ItemStack itemStack = TCOTS_EntitiesUtil.getItemFromStack(item);
    ...
    private void tickEatingItem(){
        ...
        if(this.getEatingTime() < this.getTotalEatingTime() && this.getEatingTime()!=-1){
            final ItemStack foodStack = this.getItemInHand(InteractionHand.MAIN_HAND);
            this.addEatingTime();
    ...
    private void handleEndsFeed(final ItemStack foodStack) {
    ...
    public static boolean canSpawnGhoul(final EntityType<? extends NecrophageMonster> type, final ServerLevelAccessor world, final MobSpawnType spawnReason, final BlockPos pos, final RandomSource random) {
    ...
    public SpawnGroupData finalizeSpawn(final ServerLevelAccessor world, @NotNull final DifficultyInstance difficulty, @NotNull final MobSpawnType spawnReason, @Nullable final SpawnGroupData entityData) {
        ...
        if(!(spawnReason == MobSpawnType.SPAWN_EGG) && !(spawnReason == MobSpawnType.STRUCTURE)) {
            ...
            if (random.nextInt() % 5 == 0) {
                final AlghoulEntity alghoul = TCOTS_Entities.Alghoul().create(this.level());
                if (alghoul != null) {
    ...
    public void setHasCooldownForRegen(final boolean hasCooldownForRegen) {
    ...
    public void setIsRegenerating(final boolean isRegenerating) {
    ...
    public void setIsInvokingRegen(final boolean isRegenerating) {
        this.entityData.set(INVOKING_REGENERATING, isRegenerating);
    }

    private int cooldownForRegen;

    public void setCooldownForRegen(final int cooldownForRegen) {
    ...
    protected void setTimeForRegen(final int timeForRegen) {
    ...
    protected SoundEvent getAmbientSound() {
        if(!this.getInGround()) return TCOTS_Sounds.getSoundEvent("ghoul_idle");
        return null;
    }
