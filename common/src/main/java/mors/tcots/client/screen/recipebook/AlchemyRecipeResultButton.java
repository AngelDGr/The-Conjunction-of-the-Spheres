        ...
        if (this.isHoveredOrFocused() && playerHasRecipe) {
            ...
            if(this.recipe.getResultItem(null).getItem() instanceof WitcherPotions_Base && !(this.recipe.getResultItem(null).getItem() instanceof WitcherAlcohol_Base) && !(this.recipe.getResultItem(null).getItem() instanceof WitcherWhiteHoney)){
                ...
                if((((WitcherPotions_Base) this.recipe.getResultItem(null).getItem()).isDecoction())){
                    tooltipY=12;
                    list.add(Component.translatable("gui.tcots_witcher.tooltip.formula", recipe.getResultItem(null).getHoverName().getString()).withColor(0x41d331));
                }else {
                    tooltipY=22;
                    list.add(Component.translatable("gui.tcots_witcher.tooltip.formula", recipe.getResultItem(null).getHoverName().getString()).withStyle(textColor));
                }

                //Toxicity
                final int tox = ((WitcherPotions_Base) this.recipe.getResultItem(null).getItem()).getToxicity();
                list.add(Component.translatable("item.tcots_witcher.potion.tooltip.toxicity", tox).withStyle(ChatFormatting.DARK_GREEN));

                //Stack
                final int maxCount = this.recipe.getResultItem(null).getMaxStackSize();
                if(!(((WitcherPotions_Base) this.recipe.getResultItem(null).getItem()).isDecoction())){
                    list.add(Component.translatable("gui.tcots_witcher.tooltip.max_stack", maxCount).withStyle(ChatFormatting.DARK_BLUE));
                }
                ...
                list.add(Component.translatable("gui.tcots_witcher.tooltip.oil_damage", damage).withStyle(ChatFormatting.RED));

                //Uses
                final int uses = ((WitcherMonsterOil_Base) this.recipe.getResultItem(null).getItem()).getUses();
                list.add(Component.translatable("gui.tcots_witcher.oil.tooltip.uses", uses).withStyle(ChatFormatting.DARK_BLUE));
                ...
                final List<Component> list= new ArrayList<>();
                list.add(Component.translatable("gui.tcots_witcher.tooltip.formula", recipe.getResultItem(null).getHoverName().getString()));
                //Stack
                final int maxCount = this.recipe.getResultItem(null).getMaxStackSize();
                ...
                context.renderTooltip(textRenderer, Component.translatable("gui.tcots_witcher.tooltip.formula", recipe.getResultItem(null).getHoverName().getString()), this.getX()-20, this.getY());
            }
        }
        else{
