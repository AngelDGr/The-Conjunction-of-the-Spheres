package mors.tcots.client.screen;

import mors.tcots.TCOTS_Main;
import mors.tcots.TCOTS_OwOConfig;
import com.mojang.blaze3d.systems.RenderSystem;
import net.minecraft.client.Minecraft;
import net.minecraft.client.gui.GuiGraphics;
import net.minecraft.resources.ResourceLocation;
import net.minecraft.world.entity.player.Player;

@SuppressWarnings("unused")
public class ToxicityHudOverlay {
    private static final ResourceLocation TOXICITY_BAR_EMPTY = ResourceLocation.fromNamespaceAndPath(TCOTS_Main.MOD_ID, "hud/toxicity_bar");
    private static final ResourceLocation TOXICITY_BAR_FULL = ResourceLocation.fromNamespaceAndPath(TCOTS_Main.MOD_ID, "textures/gui/sprites/hud/toxicity_bar_filled.png");
    private static final ResourceLocation TOXICITY_BAR_FULL_DECOCTION = ResourceLocation.fromNamespaceAndPath(TCOTS_Main.MOD_ID, "textures/gui/sprites/hud/toxicity_bar_filled_decoction.png");
    private static final ResourceLocation TOXICITY_OVERLAY = ResourceLocation.fromNamespaceAndPath(TCOTS_Main.MOD_ID, "textures/gui/sprites/hud/toxicity_overlay.png");


    //xTODO: Fix chat transparency
    //xTODO: Change the toxicity overlay texture

    private static float transparency;

    private static int timerHud;

    private static boolean activeHud=false;

    private static int getXAnchor(final int scaledWith, final TCOTS_OwOConfig.ANCHORS anchor){
        return switch (anchor){
            case CENTER_DOWN,CENTER_UP -> scaledWith/2 - 61;
            case RIGHT_DOWN, RIGHT_UP -> scaledWith - 120;
            case LEFT_UP,LEFT_DOWN -> 4;
        };
    }

    private static int getYAnchor(final int scaledHeight, final TCOTS_OwOConfig.ANCHORS anchor){
        return switch (anchor){
            case CENTER_DOWN -> scaledHeight-71;
            case CENTER_UP -> 22;

            case RIGHT_UP, LEFT_UP -> 4;
            case LEFT_DOWN,RIGHT_DOWN -> scaledHeight-12;
        };
    }

    public static void onHudRender(final GuiGraphics context, final float tickDelta) {


        final Minecraft client = Minecraft.getInstance();

        if (client != null && !client.options.hideGui) {
            final int scaledWidth = context.guiWidth();
            final int scaledHeight = context.guiHeight();

            final int x = (getXAnchor(scaledWidth, TCOTS_Main.CONFIG.hud.anchor())) + TCOTS_Main.CONFIG.hud.Hud_X();
            final int y = (getYAnchor(scaledHeight, TCOTS_Main.CONFIG.hud.anchor())) + TCOTS_Main.CONFIG.hud.Hud_Y();

            if(client.player!=null){
                final Player player=client.player;

                //For fade out
                if(player.tcots$getAllToxicity()>0){
                    activeHud=true;
                    transparency=1;
                    timerHud=0;
                }
                else if(player.tcots$getAllToxicity()==0){
                    if(timerHud<20){
                        transparency=transparency-0.05f;
                        ++timerHud;
                    } else {
                        activeHud=false;
                    }
                }

                if(activeHud) {
                    final int allToxicity=player.tcots$getAllToxicity();
                    final int maxToxicity= player.tcots$setMaxToxicity();
                    RenderSystem.enableBlend();

                    context.setColor(1,1,1, transparency);
                    //EmptyBar
                    context.blitSprite(TOXICITY_BAR_EMPTY, x, y, 116, 10);
                    //Normal Toxicity Bar
                    context.blit(TOXICITY_BAR_FULL, x +10, y,
                            10.0f, 0.0f,
                            (int)(106*((float)allToxicity/(float)maxToxicity)), 10,
                            116,10);

                    //Decoction Toxicity Bar
                    context.blit(TOXICITY_BAR_FULL_DECOCTION, x +10, y,
                            10.0f, 0.0f,
                            (int)(106*((float)player.tcots$getDecoctionToxicity()/(float)maxToxicity)), 10,
                            116,10);

                    context.setColor(1,1,1,1);

                    //Toxicity Overlay
                    final float toxicityThreshold= maxToxicity*0.5f;
                    if(allToxicity>=toxicityThreshold){
                        context.setColor(1,1,1,(allToxicity-toxicityThreshold)/toxicityThreshold);
                        context.blit(TOXICITY_OVERLAY, 0, 0, -90, 0.0f, 0.0f, scaledWidth, scaledHeight, scaledWidth, scaledHeight);
                        context.setColor(1,1,1,1);
                    }

                    RenderSystem.disableBlend();
                }
            }
        }
    }
}
