...
public class ScurverEntity extends RotfiendEntity{
    public ScurverEntity(final EntityType<? extends RotfiendEntity> entityType, final Level world) {
        ...
        if (!this.level().isClientSide) {
            ...
            for (int i = 0; i <= 90; i += 18) { // Vertical spread from 0 to 90 degrees (half-sphere)
                for (int j = 0; j < 360; j += 30) { // Full horizontal spread
                    final Projectile projectileEntity = new ScurverSpineEntity(this, this.level(), new ItemStack(TCOTS_Items.SCURVER_SPINE.get()), null);

                    // Calculate the direction vector for the current angles
                    final double radianI = Math.toRadians(i+this.getRandom().nextIntBetweenInclusive(0,20));
                    final double radianJ = Math.toRadians(j+this.getRandom().nextIntBetweenInclusive(0,20));

                    final double x = Math.sin(radianI) * Math.cos(radianJ);
                    final double y = Math.cos(radianI);
                    final double z = Math.sin(radianI) * Math.sin(radianJ);

                    final Vector3f vector3f = new Vector3f((float)x, (float)y, (float)z);
