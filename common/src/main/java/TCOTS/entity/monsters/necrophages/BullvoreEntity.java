...
public class BullvoreEntity extends NecrophageMonster implements GeoEntity {
    ...
    protected static class Bullvore_MeleeAttackGoal extends Goal {
        ...
        public Bullvore_MeleeAttackGoal(final BullvoreEntity mob, final double speed, final boolean pauseWhenMobIdle, final double speedMultiplier, final int chargeCooldown) {
            this.mob = mob;
            this.speed = speed;
        ...
        protected void attack(final LivingEntity target) {
            if (this.canAttack(target)) {
                ...
                final int randomAttack = this.mob.getRandom().nextIntBetweenInclusive(0, 2);
                if (randomAttack == 0) {
