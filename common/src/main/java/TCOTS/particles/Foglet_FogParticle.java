...
public class Foglet_FogParticle extends TextureSheetParticle {
    ...
    private Foglet_FogParticle(final ClientLevel world, final double x, final double y, final double z, final double velocityX, final double velocityY, final double velocityZ, final SpriteSet spriteProvider) {
    ...
    public float getQuadSize(final float tickDelta) {
        return this.quadSize
                *
                (4f-(this.age* ((float) 4/this.lifetime)));
    }
    ...
    public void tick() {
        ...
        if (!this.removed) {
            ...
            final Player playerEntity = this.level.getNearestPlayer(this.x, this.y, this.z, 1.0, false);

            if (playerEntity != null && this.y > (d = playerEntity.getY()) && !(playerEntity.hasEffect(TCOTS_Effects.FogletDecoctionEffect()))) {
                this.y += (d - this.y) * 0.2;
    ...
    public static class FogFactory implements ParticleProvider<SimpleParticleType> {
        ...
        public FogFactory(final SpriteSet spriteProvider) {
            this.spriteProvider = spriteProvider;
        }
